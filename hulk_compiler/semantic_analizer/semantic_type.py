...
class Type:
    ...
    def set_parent(self, parent):
        if (
            isinstance(parent, StringType)
            or isinstance(parent, NumberType)
            or isinstance(parent, BooleanType)
        ):
    ...
    def __str__(self):
        if self.is_protocol:
            ...
            parent = (
                ""
                if self.parent is None
                else " extends" + ", ".join(parent.name for parent in self.parent)
            )
            output += parent
            output += " {"
...
class Attribute:
    ...
    def __str__(self):
        return (
            f"[attrib] {self.name}" + " : {self.type.name}"
...
class NumberType(Type):
    ...
    def conforms_to(self, other):
        return (
            ...
            or isinstance(other, ObjectType)
        )
...
class RangeType(Type):
    def __init__(self):
        ...
        self.methods = [
            Method("current", [], [], NumberType(), -1),
            Method("next", [], [], BooleanType(), -1),
        ]
        self.parent = ObjectType()

        self.attributes = [
            Attribute("min", NumberType),
            Attribute("max", NumberType),
...
class VectorType(Type):
    ...
    def __eq__(self, other):
        return (
            other
            and isinstance(other, VectorType)
...
class IterableType(Type):
    def __init__(self, item_type=None):
        ...
        self.methods = [
            Method("current", [], [], item_type or ObjectType(), -1),
            Method("next", [], [], BooleanType(), -1),
        ]
