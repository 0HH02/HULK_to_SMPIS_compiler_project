...
class TypeInferer(IVisitor):
    ...
    def infer_node(node: Program, context: Context):
        for _ in range(2):
            for definition in node.defines:
                TypeInferer.infer_node(definition, context)

        TypeInferer.infer_node(node.statement, context)

    @staticmethod
    @dispatch(LiteralNode, Context)
    def infer_node(_: LiteralNode, __: Context):
        """
    ...
    def infer_node(node: NegativeNode, context: Context):
    ...
    def infer_node(node: PositiveNode, context: Context):
        node.inferred_type = NumberType()
    ...
    def infer_node(node: NotNode, context: Context) -> bool:
    ...
    def infer_node(node: ExpressionBlock, context: Context):

        for expression in node.body:
            TypeInferer.infer_node(expression, context)

        node.inferred_type = node.body[-1].inferred_type

    @staticmethod
    @dispatch(While, Context)
    def infer_node(node: While, context: Context):

        TypeInferer.infer_node(node.condition, context)
        infer_top_bottom(node.condition, BooleanType(), context)

        TypeInferer.infer_node(node.body, context)
        node.inferred_type = node.body.inferred_type

    @staticmethod
    @dispatch(Elif, Context)
    def infer_node(node: Elif, context: Context):
    ...
    @staticmethod
    @dispatch(If, Context)
    def infer_node(node: If, context: Context):

        TypeInferer.infer_node(node.condition, context)
        infer_top_bottom(node.condition, BooleanType(), context)

        for elif_c in node.elif_clauses:
            TypeInferer.infer_node(elif_c, context)

        TypeInferer.infer_node(node.else_body, context)
        TypeInferer.infer_node(node.body, context)
    ...
    def infer_node(node: For, context: Context):

        TypeInferer.infer_node(node.iterable, context)
        ...
        TypeInferer.infer_node(node.body, new_context)

        node.inferred_type = node.body.inferred_type

    @staticmethod
    @dispatch(Vector, Context)
    def infer_node(node: Vector, context: Context):

        for element in node.elements:
            TypeInferer.infer_node(element, context)
    ...
    def infer_node(node: ComprehensionVector, context: Context):

        TypeInferer.infer_node(node.iterator, context)
        ...
        if context.iter_protocol.is_implemented_by(node.iterator.inferred_type):
            ...
            TypeInferer.infer_node(node.generator, child_context)

            node.inferred_type = VectorType(node.generator.inferred_type)

    @staticmethod
    @dispatch(IndexNode, Context)
    def infer_node(node: IndexNode, context: Context):

        TypeInferer.infer_node(node.obj, context)
        TypeInferer.infer_node(node.index, context)
    ...
    def infer_node(node: FunctionCall, context: Context):
        ...
        for i, arg in enumerate(node.invocation.arguments):
            TypeInferer.infer_node(arg, context)
    ...
    def infer_node(node: Invocation, context: Context):
    ...
    def infer_node(node: AttributeCall, context: Context):
    ...
    def infer_node(node: DestructiveAssign, context: Context):

        TypeInferer.infer_node(node.identifier, context)
        TypeInferer.infer_node(node.expression, context)
    ...
    def infer_node(node: Identifier, context: Context):
    ...
    def infer_node(node: LetVar, context: Context) -> bool:
        ...
        for var_declaration in node.declarations:
            ...
            TypeInferer.infer_node(var_declaration, context)
            new_context.define_variable(
    ...
    def infer_node(node: VariableDeclaration, context: Context):
    ...
    def infer_node(node: Instanciate, context: Context) -> bool:
        ...
        if type_t:
            ...
            print(f"type {node.identifier} is not defined")

        for i, param in enumerate(node.params):
            TypeInferer.infer_node(param, context)
            infer_top_bottom(param, type_t.params[i].type, context)

    @staticmethod
    @dispatch(BinaryExpression, Context)
    def infer_node(node: BinaryExpression, context: Context):
        ...
        if node.operator in [
            ...
            TypeInferer._visit_binary_aritmethic(node, context)

        if node.operator in [Operator.AND, Operator.OR]:
            TypeInferer._visit_binary_logic(node, context)
        ...
        if node.operator in [
            ...
            TypeInferer._visit_binary_comparison(node, context)

        if node.operator is Operator.IS:
            TypeInferer._visit_binary_type_checker(node, context)

        if node.operator is Operator.AS:
            TypeInferer._visit_binary_downcast(node, context)

        if node.operator in [Operator.CONCAT, Operator.DCONCAT]:
            TypeInferer._visit_binary_concat(node, context)

    @staticmethod
    def _visit_binary_aritmethic(node: BinaryExpression, context: Context):

        TypeInferer.infer_node(node.left, context)
        TypeInferer.infer_node(node.right, context)
    ...
    @staticmethod
    def _visit_binary_logic(node: BinaryExpression, context: Context):
    ...
    @staticmethod
    def _visit_binary_comparison(node: BinaryExpression, context: Context):
    ...
    @staticmethod
    def _visit_binary_downcast(node: BinaryExpression, context: Context):
    ...
    @staticmethod
    def _visit_binary_type_checker(node: BinaryExpression, context: Context):
    ...
    def infer_node(_: ProtocolDeclaration, __: Context):
        """
    ...
    def infer_node(node: FunctionDeclaration, context: Context):
        ...
        TypeInferer.infer_node(node.body, fun_context)
    ...
    def infer_node(node: TypeDeclaration, context: Context):
        ...
        if node.inherits:
            TypeInferer.infer_node(node.inherits, constructor_context)

        for function in node.functions:
            type_context.define_method(type_t.get_method(function.identifier))

        for attr in node.attributes:
            TypeInferer.infer_node(attr, constructor_context)
        ...
        for function in node.functions:
            ...
            TypeInferer.infer_node(function, funtion_context)
    ...
    def infer_node(node: AttributeDeclaration, context: Context):
        TypeInferer.infer_node(node.expression, context)

    @staticmethod
    @dispatch(Inherits, Context)
    def infer_node(node: Inherits, context: Context):
